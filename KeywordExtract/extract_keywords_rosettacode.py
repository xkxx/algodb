...
from code_keyword_extractor import extract_keywords

from nel_code_keywords import link_implementation
        ...
        if page.page_title not in indexedimpl or UPDATING:  # save time
            ...
            if algo_ids is None:
                ...
                for impl in pagetask.solutions:
                     if impl['language'] == 'Python':
                         keywords = get_code_keywords(impl['content'], 'python')
            ...
            if algo_ids is not None:
                ...
                if len(algo_ids) > 0:
                    if pagetask is None:
                        pagetask = Task(page)
                    index_rosetta_page(pagetask, algo_ids)

def index_rosetta_page(pagetask, algo_ids):
    for impl in pagetask.solutions:
        body = {
        ...
        if impl['language'] == 'Python':
            body['code_keywords'] = get_code_keywords(impl['content'], 'python')
...
def get_code_keywords(content, lang):
    keywords = {}
    for entry in content:
        if entry['type'] == 'code':
            keyword_list = extract_keywords(entry['content'], lang)
            for (keywordtype, keyword) in keyword_list:
                old_list = keywords.get(keywordtype, list())
                old_list.append(keyword)
                keywords[keywordtype] = old_list
